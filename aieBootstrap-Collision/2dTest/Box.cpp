...
Box::Box(vec2 position, float inclination, float speed, float length, float height, float mass, float linearDrag, float angularDrag, float elasticity, vec4 color) :Box(position,length,height, vec2(cosf(inclination), sinf(inclination))*speed, mass, linearDrag,angularDrag,elasticity,color)
{
}
...
vec2 Box::getExtents()
{
	return vec2(m_position.x + m_length, m_position.y + m_height);
}